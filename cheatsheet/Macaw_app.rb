cheatsheet do
  title 'Macaw'
  docset_file_name 'Macaw_app'
  keyword 'macaw'
  source_url 'http://cheat.kapeli.com'

  category do
    id 'File'

    entry do
      command 'CMD+I'
      name 'Import to Canvas…'
      notes 'Import assets directly on your canvas'
    end

    entry do
      command 'CMD+SHIFT+I'
      name 'Import to Library…'
    end

    entry do
      command 'CMD+P'
      name 'Publish Page'
    end

    entry do
      command 'CMD+ALT+P'
      name 'Publish Project'
    end

    entry do
      command 'CMD+SHIFT+P'
      name 'Publish Settings…'
    end

  end

  category do
    id 'Edit'

    entry do
      command 'ALT+CMD+C'
      name 'Copy Visual Properties'
    end

    entry do
      command 'ALT+CMD+V'
      name 'Paste Visual Properties'
    end

    entry do
      command 'CMD+D'
      name 'Duplicate'
    end

    entry do
      command 'CMD+['
      name 'Move Backwards'
    end

    entry do
      command 'SHIFT+CMD+['
      name 'Send to Back'
    end

    entry do
      command 'CMD+]'
      name 'Move Forward'
    end

    entry do
      command 'SHIFT+CMD+]'
      name 'Send to Front'
    end

    entry do
      command 'CMD+R'
      name 'Rename'
    end
  end

  category do
    id 'View'

    entry do
      command 'CMD+Y'
      name 'Toggle Outline Mode'
    end

    entry do
      command 'CMD+W'
      name 'Toggle Wireframe Mode'
    end

    entry do
      command 'ALT+CMD+0'
      name 'Center Canvas'
    end

    entry do
      command 'CMD+;'
      name 'Toggle Grid'
    end

    entry do
      command 'CMD+U'
      name 'Toggle Snap to Objects'
    end

    entry do
      command 'Tab'
      name 'Toggle Palettes'
    end

    entry do
      command 'SHIFT+['
      name 'View Lower Breakpoint'
    end

    entry do
      command 'SHIFT+]'
      name 'View Higher Breakpoint'
    end

    entry do
      command 'SHIFT+\\'
      name 'Toggle to Default Breakpoint'
    end
  end

  category do
    id 'Tools'

    entry do
      command 'V'
      name 'Selection Tool'
    end

    entry do
      command 'A'
      name 'Direct Select Tool'
      notes 'by pressing CMD + click you can use this tool when you are using the Selection Tool'
    end

    entry do
      command 'T'
      name 'Text Tool'
    end

    entry do
      command 'R'
      name 'Element Tool'
    end

    entry do
      command 'G'
      name 'Container Tool'
    end

    entry do
      command 'B'
      name 'Button Tool'
    end

    entry do
      command 'N'
      name 'Input Tool'
    end

    entry do
      command 'M'
      name 'Embed Tool'
    end

    entry do
      command 'H'
      name 'Hand Tool'
    end

    entry do
      command 'I'
      name 'Eyedropper Tool'
    end
  end

  category do
    id 'Window'

    entry do
      command 'ALT+I'
      name 'Show Inspector'
    end

    entry do
      command 'ALT+O'
      name 'Show Outline'
    end

    entry do
      command 'ALT+L'
      name 'Show Library'
    end

    entry do
      command 'P'
      name 'Show Pages'
    end

    entry do
      command 'C'
      name 'Show Color Picker'
    end

    entry do
      command 'S'
      name 'Show Quick Swatches'
    end

    entry do
      command 'F8'
      name 'Show Scripts'
    end

    entry do
      command 'Q'
      name 'Show Queries'
    end
  end

  category do
    id 'Elements'

    entry do
      command 'CMD+G'
      name 'Tight Group'
    end

    entry do
      command 'ALT+CMD+G'
      name 'Full-Width Group'
    end

    entry do
      command 'SHIFT+CMD+G'
      name 'Ungroup'
    end

    entry do
      command 'CMD+T'
      name 'Transform'
    end

    entry do
      command 'CMD+E'
      name 'Expand'
    end

    entry do
      command 'CMD+F'
      name 'Fit to Children'
    end

    # entry do
    #   command 'CMD+M'
    #   name 'Morph…'
    # end

    entry do
      command 'ALT+CMD+R'
      name 'Refresh All Assets'
    end

    # entry do
    #   command 'CMD+3'
    #   name 'Hide Selected'
    # end

    # entry do
    #   command 'ALT+CMD+3'
    #   name 'Show All'
    # end

    entry do
      command 'CMD+2'
      name 'Lock Selected'
    end

    entry do
      command 'ALT+CMD+2'
      name 'Unlock All'
    end

    entry do
      command 'CMD+K'
      name 'Create Component…'
    end

    entry do
      command 'CMD+B'
      name 'Break Component Reference'
    end

    entry do
      command 'Arrow Right'
      name 'Nudge 1px'
    end

    entry do
      command 'SHIFT+Arrow Right'
      name 'Nudge 10px'
    end

    entry do
      command 'CMD+Arrow Right'
      name 'Nudge to Grid'
    end

    entry do
      command 'ALT+Arrow Right'
      name 'Pudge 1px'
    end

    entry do
      command 'SHIFT+ALT+Arrow Right'
      name 'Pudge 10px'
    end

    entry do
      command 'ALT+CMD+Arrow Right'
      name 'Pudge to Grid'
    end
  end

  notes <<-'END'
  * Based on the official cheat sheet.
  * Converted by [Nicolas Genty](https://twitter.com/ngenty)
  END
end
